...
var updateGPS = setInterval(function(){
    getLocation(function(a){    
        ...
        UserMarker.start();/*
        if(!latlngs_first){
            var r = distance(latlngs[0],latlngs[1],latlngs[2],latlngs[3]);
            console.log(r);
            if(r < 1){
                //UserMarker.addLatLng([latlngs[0]+ppp,latlngs[1]+qqq],[500]);
                //UserMarker.start();
            }else if(r<5){
                UserMarker.moveTo([latlngs[0]+ppp,latlngs[1]+qqq],[500]);
                UserMarker.start();
            }else{
                UserMarker.moveTo([latlngs[0]+ppp,latlngs[1]+qqq],[5000]);
                UserMarker.start();
            }
            latlngs[2] = latlngs[0];
            latlngs[3] = latlngs[1];
...
function upRightMenu(attr,cb){
    ...
    //詳細資料
    e1.value = attr.SiteName || "無";
    e2.value = t0 ;
    e3.value = attr.Device_id;
    e4.value = attr.Humidity + '%';
    e5.value = (PtLayID==4 || PtLayID==5) ? "無" : attr.Pm1/10.0 ;
    e6.value = attr.Pm25/10.0;
    e7.value = (PtLayID==4 || PtLayID==5) ? "無" : attr.Pm10/10.0;
    e8.value = (PtLayID==0) ? attr.Voltage : "無";
    e9.value = (PtLayID<2) ? attr.Satellites : "無";
    ea.value = (PtLayID==1) ? attr.Barometer/10.0 : "無";
    ...
    eb.innerText = " "+ye+"/"+mo+"/"+da+" "+ho+":"+mi+":"+se;
...
window.getLassProg = setInterval(function(){
    $.getJSON('https://u9923001.myddns.me/sensor/4', function(data) {
...
//socket.on('who', function(data) {
    /*
    var a={lat:UserMarker._latlng.lat, lng:UserMarker._latlng.lng};
    socket.emit('me',a.lat+','+a.lng);
    console.log(a);*/
    /*
...
//});
//LASS感測資料接收
/*function getLass(id,data) {
    SkioState[id] = true;
    LassData[id]=data;
    upLaPop(id,LassData[id]);
